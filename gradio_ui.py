...
def image_recognition(image, api_key):
    ...
    prompt = (
        ...
        原則2：輸出格式(但不用出現本句)：$bank:| 銀行名字 $account:| 銀行帳號 $name:| 名字 $none:| none\
    ...
    print(result)
    qname = result.split("$bank:|")[1].split("$account:|")[0].strip()
    qfee = result.split("$account:|")[1].split("$name:|")[0].strip()
    qdate = result.split("$name:|")[1].split("$none:|")[0].strip()
